...
def iterator(number_passes, one_pass, inputkey, opername, oper, functioncolumn):
	# one pass
	if number_passes > 1:
...
def SQL():
	...
	while diff > threshold and iter < number_passes:
		...
		starttime = time.time()
		difference = []
		result = iterator(number_passes, one_pass, inputkey, opername, oper, functioncolumn)
		for r in result:
	...
	yield render_template('temp.html',items=record,iter="Result",Key=inputkey[1:],Value=opername,Log="True")
